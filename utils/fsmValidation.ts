...
function checkEventListeners(
  ...
  listeners.forEach(listener => {
    ...
    // 检查动作中的脚本引用（InvokeScript）
    if (listener.action?.type === 'InvokeScript' && listener.action.scriptId) {
      if (isScriptMarkedForDelete(state, listener.action.scriptId)) {
        issues.push({
          type: 'error',
          message: `Listener action references deleted script: ${listener.action.scriptId}`,
          resourceType: 'script',
          resourceId: listener.action.scriptId
        });
      }
    }
...
export function checkStateValidation(
  ...
  if (isScriptMarkedForDelete(editorState, stateNode.onEnterScriptId)) {
    issues.push({
      type: 'error',
      message: `OnEnter script is deleted: ${stateNode.onEnterScriptId}`,
      resourceType: 'script',
      resourceId: stateNode.onEnterScriptId!
    });
  }

  if (isScriptMarkedForDelete(editorState, stateNode.onExitScriptId)) {
    issues.push({
      type: 'error',
      message: `OnExit script is deleted: ${stateNode.onExitScriptId}`,
      resourceType: 'script',
      resourceId: stateNode.onExitScriptId!
    });
  }
