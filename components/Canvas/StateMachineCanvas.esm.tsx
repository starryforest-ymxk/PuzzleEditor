...
if (!fsm)
    return _jsx("div", { className: "empty-state", children: "Missing Data" });
return (_jsxs("div", { ref: canvasRef, className: "canvas-grid", style: {
        ...
        cursor: isPanningActive ? 'grabbing' : (linkingState || modifyingTransition ? 'crosshair' : (boxSelectRect ? 'crosshair' : 'default'))
    }, onMouseDown: handleCanvasMouseDown, onMouseUp: handleCanvasMouseUp, onContextMenu: (e) => handleContextMenu(e, 'CANVAS'), children: [_jsx(CanvasInfoOverlay, { nodeName: node.name, multiSelectCount: multiSelectIds.length, isLineCuttingMode: isLineCuttingMode, isLinkMode: Boolean(linkingState), isPanMode: isPanningActive }), _jsx(ShortcutPanel, { visible: showShortcuts, onToggle: () => setShowShortcuts(v => !v) }), contextMenu && (_jsx(CanvasContextMenu, { menu: contextMenu, onClose: () => setContextMenu(null), onAddState: (x, y) => dispatch({
                type: 'ADD_STATE',
                payload: {
                    fsmId: fsm.id,
                ...
                payload: { fsmId: fsm.id, transitionId }
            }), isInitialState: contextMenu.type === 'NODE' && fsm.initialStateId === contextMenu.targetId, contentRef: contentRef })), _jsxs("div", { ref: contentRef, style: { position: 'relative', minWidth: `${CANVAS_SIZE}px`, minHeight: `${CANVAS_SIZE}px` }, children: [_jsx(BoxSelectOverlay, { rect: boxSelectRect }), _jsxs("svg", { style: { position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', overflow: 'visible', pointerEvents: 'none', zIndex: 0 }, children: [_jsx(ConnectionArrowMarkers, {}), Object.values(fsm.transitions).map((trans) => {
                            ...
                            return _jsx("path", { d: Geom.getBezierPathData(p1, p2, sSide, eSide), fill: "none", stroke: "#888", strokeWidth: "2", strokeDasharray: "5,5", markerEnd: "url(#arrow-temp)" });
                        })(), _jsx(CuttingLineOverlay, { line: cuttingLine })] }), Object.values(fsm.transitions).map((trans) => {
                    ...
                    return (_jsx(ConnectionControls, { transition: trans, fromPos: fromPos, toPos: toPos, isSelected: ui.selection.type === 'TRANSITION' && ui.selection.id === trans.id, isContextTarget: contextMenu?.type === 'TRANSITION' && contextMenu?.targetId === trans.id, onSelect: (e, id) => dispatch({ type: 'SELECT_OBJECT', payload: { type: 'TRANSITION', id, contextId: node.id } }), onContextMenu: (e, id) => handleContextMenu(e, 'TRANSITION', id), onHandleDown: (e, id, type) => {
                            e.stopPropagation();
                            startModifyingTransition(e, id, type);
                        }, readOnly: readOnly }, `ctrl-${trans.id}`));
                }), _jsx(SnapPointsLayer, { snapPoints: snapPoints, activeSnapPoint: activeSnapPoint, visible: Boolean(linkingState || modifyingTransition) }), Object.values(fsm.states).map((state) => (_jsx(StateNode, { state: state, position: getNodeDisplayPosition(state.id, state.position), isSelected: ui.selection.type === 'STATE' && ui.selection.id === state.id, isMultiSelected: multiSelectIds.includes(state.id), isInitial: fsm.initialStateId === state.id, isContextTarget: contextMenu?.type === 'NODE' && contextMenu?.targetId === state.id, onMouseDown: handleStateMouseDown, onMouseUp: handleStateMouseUp, onContextMenu: (e) => handleContextMenu(e, 'NODE', state.id), readOnly: readOnly }, state.id)))] }), _jsx("style", { children: ` .ctx-item { padding: 8px 12px; font-size: 13px; cursor: pointer; color: #eee; } .ctx-item:hover { background: #3e3e42; } .ctx-item.danger { color: #ff6b6b; } .handle { position: absolute; width: 12px; height: 12px; background: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255,255,255,0.5); border-radius: 50%; transform: translate(-50%, -50%); cursor: grab; opacity: 0; transition: opacity 0.2s, background 0.2s; } .handle:hover { opacity: 1; background: var(--accent-color); border-color: #fff; } ` })] }));
;
