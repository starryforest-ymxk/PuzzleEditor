...
- 在 Transition Inspector 中集成完整的条件构造 UI：
  ...
  - 支持添加自定义条件脚本引用；
  - 支持 AND/OR 组合以及分组嵌套。
- 变量选择器要展示 Global / Stage / Node 作用域，并根据当前上下文自动过滤；类型不匹配时给出错误提示。
...
- 为 FSM 画布补齐完整的交互：框选、多选、批量移动、对齐辅助线等。
- 实现局部即时校验：
  - 没有初始状态时在画布边缘显示提示；
